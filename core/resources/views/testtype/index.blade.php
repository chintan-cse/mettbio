...
<section class="main-content-section style-8">
	  ...
	  <div class="main-panel">
		  ...
		  <div class="page-header">
              <h3 class="page-title">
                ...
                </span> Test Type Methods </h3>
              <nav aria-label="breadcrumb">
                <ul class="breadcrumb">
                  <li class="breadcrumb-item active" aria-current="page">
                   <span class="">
						<a href="{{route('home')}}"><i class="mdi mdi-home"></i></a></span><span><i class="mdi mdi-arrow-right"></i> Typetest Methods</span> 
                  </li>
                </ul>
              </nav>
            </div>
	...
	<div id="myModal" class="modal fade users-modal" role="dialog">
	  <div class="modal-dialog modal-lg">
        ...
        <div class="page-header">
		<h5 class="modal-title req-popup-title">Type Detail</h5>
...
$(document).on( "click",'.clickname', function() {
	...
	var url = '{{ route("testtype.show", ":id") }}';
...
//multiple records delete starts
	...
	$(document).on('click', '#multiplerequestdelete', function(){
        ...
        // {
            ...
            if(id.length > 0)
            {
				if(confirm("Are you sure you want change the status of this Test Type?"))
				{
					$.ajax({
						url:"{{ route('ajaxdata.masstesttypedelete')}}",
						method:"get",
						data:{id:id},
						datatype: 'JSON',
...
$("#requestlist-table").DataTable({
		...
		fnDrawCallback: function() {
			...
			$('.toggleitem').change(function() {				
				...
				var url = '{{ route("testtype.destroy", ":id") }}';
				url = url.replace(':id', dataid);
